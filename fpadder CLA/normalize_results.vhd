-- 6.1.7. comp6 - normalize_results.vhd
...
entity normalize_result is
    port( Maybe_Subnormal : in  STD_LOGIC_VECTOR (36 downto 0);  
          exp_underflow   : out  STD_LOGIC; 
          Normal          : out  STD_LOGIC_VECTOR (36 downto 0));
end normalize_result;

architecture Behavioral of normalize_result is
...
signal exp_underflow_s  : STD_LOGIC;
signal exp_A_s1,exp_A_s2 : STD_LOGIC_VECTOR (7 downto 0);

begin

Mantissa_A_s <= Maybe_Subnormal (27 downto 0);
exp_A_s1 <= Maybe_Subnormal (35 downto 28);
...
"-----";                                    
process (Maybe_Subnormal, Mantissa_A_s,x_times_leftshift_s,exp_A_s1)
    ...
    begin
        if x_times_leftshift_s > exp_A_s1 then
            exp_underflow_s <= '1';
            exp_A_s2 <= "00000000";
            shifted_A_s <= "----------------------------";
        elsif   x_times_leftshift_s <= exp_A_s1 then
            exp_A_s2 <= exp_A_s1 - x_times_leftshift_s;
            exp_underflow_s <= '0';
                --Shifting
                -- 1st shift stage
...
exp_underflow <= exp_underflow_s;
Normal (36) <= Maybe_Subnormal(36);  -- Sign
Normal (35 downto 28) <= exp_A_s2;   -- Increased exponent
Normal (27 downto 0) <= shifted_A_s; -- Shifted Mantissa
end Behavioral;
